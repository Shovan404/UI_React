...
usernameChangeHandler = (event) =>{
	...
	if(event.target.value.length < 4){
		...
		this.setState({validationMessageUsername: "Valid username"});
	}
}
...
passwordChangeHandler = (event) =>{
	...
	if(event.target.value.length < 10){
		...
		this.setState({validationMessage: "Valid password"});
	}
}
...
SubmitHandler = (event) =>{
	...
	Axios.post('http://localhost:3000/login', this.state, headers)
	.then(function(res){
